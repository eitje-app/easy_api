...
const handleProgress = ({loaded = 0, total = 0}) => {
 ...
 export const upload = async (data, {method = 'post', paramName = 'file', doLoad, setProgress = () => {}, url} = {} ) => {
  ...
  let condHeaders = {}
  if(doLoad) condHeaders['doLoad'] = true;

  return func(url, fileData, {
                                 onUploadProgress: (info) => setProgress(handleProgress(info)),  
                                 headers: {...condHeaders, 'Accept': 'application/json', 'Content-Type': 'multipart/form-data'}} )
