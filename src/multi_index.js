...
export const multiIndex = async (resources) => {
  ...
  if (res.ok) {
    ...
    data = data || {};
    const { items = {} } = data;
    let reduxPayload = Object.keys(items).map((k) =>
      transformMultiResponse(items[k], localObj[k])
    );
    reduxPayload = reduxPayload.filter(Boolean);
    if (utils.exists(reduxPayload)) {
      config.store.dispatch({
        type: "MULTI_INDEX",
        payload: reduxPayload.filter(Boolean),
      });
    }
  }
};
...
const buildResourceParams = (props) => {
  ...
  const url = config.indexUrls[name]
    ? funcOrValue(config.indexUrls[name])
    : `${name}/index`;
  ...
  const {
    ...
    currentItems = [],
    allItems = [],
  } = refresh ? {} : newGetStamps({ name, inverted, cacheName });
