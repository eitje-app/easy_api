...
const extendMulti = (args) => {
  const { items, mergeItemLeading, fieldName, mergeTableName, mergeItems } =
    args;
  const res = mergeItemLeading
    ? extendMultiMergeLeading(args)
    : extendMultiMainLeading(args);
  return res;
};

const extendMultiMainLeading = ({
  items,
  mergeItemLeading,
  fieldName,
  mergeTableName,
  ...
  return items.map((i) => {
    const relevantItems = i[fieldName]
      .map((i2) => mergeItemsIndex[i2])
      .filter(Boolean);
    return { ...i, [pluralName]: relevantItems };
  });
};

const extendMultiMergeLeading = ({
  items,
  fieldName,
  mergeItemLeading,
  mergeTableName,
