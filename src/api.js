...
export { multiIndex };
const { store, indexUrls, createUrls, updateUrls, deleteUrls, afterAdd } =
  config;
...
export async function index(
  ...
  const url = config.indexUrls[kind]
    ? funcOrValue(config.indexUrls[kind])
    : `${kind}/index`;
  ...
  const {
    ...
    : getStamps(
        ...
        params,
        inverted,
        overrideCacheKind || cacheKind
      );

  let currentIds = currentItems.map((i) => i.id);
  const unscopedIds = allItems
    .filter((i) => !utils.exists(i.fetchedKinds))
    .map((i) => i.id); // we wanna include pushered and other non-fetched items to ensure they still exist
  ...
  if (res.ok) {
    ...
    mappedItems = afterIndex(kind, items, {
      localKind: overrideCacheKind || cacheKind,
    });
    if (
      items.length > 0 ||
      hasForce ||
      destroyed_ids.length > 0 ||
      removed_from_scope_ids.length > 0
    ) {
...
export async function multiDestroy(kind, ids, { extraParams = {} } = {}) {
  const res = await backend.post(`${kind}/multi_destroy`, {
    ids,
    ...extraParams,
  });
...
export async function updateMulti(
  kind,
  params,
  { localKind, extraParams, saveLocal = true } = {}
) {
  const res = await backend.post(`${kind}/multi_update`, {
    items: params,
    ...extraParams,
  });
  return handleRes(res, kind);
...
const sanitizeKind = (kind) =>
  kind ? pluralize(utils.camelToSnake(kind)) : kind;
...
export async function destroy(kind, id, extraParams = {}) {
  const url = config.deleteUrls[kind]
    ? funcOrValue(config.deleteUrls[kind], id)
    : `${kind}/${id}`;
...
export async function resourceReq(kind, url, config = {}) {
  ...
  const fullUrl = isCreate
    ? `${backendKind}/${url}`
    : `${backendKind}/${params["id"]}/${url}`;
...
export async function addMulti(
  kind,
  params,
  { extraParams = {}, showSucc = true } = {}
) {
  const obj = {};
  obj[kind] = params;
  const res = await backend.post(`${kind}/add_multi`, {
    ...obj,
    ...extraParams,
  });
  if (res.ok) {
...
export async function createMultiLocal(kind, items, { localKind } = {}) {
  ...
  config.store.dispatch({
    type: "LOCAL_INDEX_RECORDS",
    items,
    kind: utils.snakeToCamel(actKind),
  });
}
...
export async function updateMultiPartial(kind, params) {
  ...
  const items = utils.findAndReplace({
    oldItems: state.records[kind],
    newItems: params,
  });
  createMultiLocal(kind, items);
}
...
export async function removeRelaton(kind, id, relName, value) {
  ...
  item &&
    createLocal(kind, {
      ...item,
      [relName]: item[relName].filter((v) => v != value),
    });
}
