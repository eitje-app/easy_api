...
export const sanitizeMoment = (v) =>
  v instanceof moment ? v.format("YYYY-MM-DD") : v;
...
export function deepTransformValues(obj, mapper) {
  for (let key in obj) {
    if (typeof obj[key] === "object" && obj[key] !== null) {
      obj[key] = moment.isMoment(obj[key])
        ? mapper(obj[key])
        : deepTransformValues(obj[key], mapper);
    } else {
...
const serializeNestedParams = (params) =>
...
async function changeTokenHeader(req) {
  if (
    req.url !== "oauth/token" &&
    req.url !== "auth" &&
    req.url !== "users/sign_up" &&
    req.url !== "auth/confirmed"
  ) {
