...
export const filterRelevant = (kind, items, extraParams = {}, cacheKind) => {
  items = items.filter(
    (i) => i.fetchedKinds && i.fetchedKinds.includes(cacheKind)
  );
  return config.filterStampItems
    ? config.filterStampItems(kind, items, extraParams)
    : items;
};
...
export const makeCacheKind = (kind, filters) => {
  ...
  const sortedStringified = JSON.stringify(
    sanitizedFilters,
    Object.keys(filters).sort()
  ); // we sort to ensure order of keys doesn't matter
...
export const newGetStamps = ({ name, inverted, cacheName }) => {
  ...
  let items = allItems.filter(
    (i) => i.fetchedKinds && i.fetchedKinds.includes(cacheName)
  );
...
export const newAfterIndex = ({ cacheName, items = [] }) => {
  ...
  items = items.map((i) => ({
    ...i,
    fetchedKinds: _.uniq([...(i.fetchedKinds || []), cacheName]),
  }));
  return items;
};
...
export const afterIndex = (kind, items = [], { localKind }) => {
  ...
  items = items.map((i) => ({
    ...i,
    fetchedKinds: _.uniq([...(i.fetchedKinds || []), localKind]),
  }));

  return config.afterIndex
